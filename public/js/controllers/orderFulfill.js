...
angular.module('orderFulfill', [
    ...
    $scope.createShipment = function() {
      ...
      Shipment.create({}, {
        ...
        items: _.map($scope.newShipment.items, function (item) {
          if (item.type === 'printer' || item.type === 'tablet' ||item.type === 'scanner') {
        ...
        .then(function (shipment) {
          ...
          $scope.order.items = _.map($scope.order.items, function (item) {
            var itemsToBeShipped = _.union(machinesToBeShipped, filamentsToBeShipped, cartridgesToBeShipped);
        ...
        .then(function (order) {
          ...
          return $q.all(_.union(machineUpdates, cartridgeUpdates, filamentUpdate));
        })
  ...
  .controller('findItemModal', function ($q, $log, $scope, $modalInstance, type, Machine, Filament, Cartridge) {
    $scope.type = type;
          ...
          if (item.machineType) {
            if (item.machineType === type) {
              if (item.machineStatus === 'Operational in warehouse') {
                ...
                return $q.reject('Machine not ready to ship, its status is ' + item.machineStatus);
              }
            } else {
              return $q.reject('machine type does not match. It is a ' + item.machineType);
            }
          } else {
            $modalInstance.close(item);
          }
